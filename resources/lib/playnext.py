        ...
        while not xbmc.Monitor().abortRequested() and not self.stop_thread:
            ...
            if player.isPlaying():
                ...
                if not is_playing:
                    settings = xbmcaddon.Addon()
                    play_next_trigger_time = int(settings.getSetting('play_next_trigger_time'))
                    log.debug(f"New play_next_trigger_time value: {play_next_trigger_time}")
                ...
                if not play_next_triggered and (trigger_time > time_to_end) and play_next_dialog is None:
                    ...
                    log.debug(f"play_next_triggered hit at {time_to_end} seconds from end")

                    play_data = get_playing_data()
                    log.debug(f"play_next_triggered play_data : {play_data}")
