...
class ActionAutoClose(threading.Thread):
    ...
    def run(self):
        ...
        while not xbmc.Monitor().abortRequested() and not self.stop_thread:
            ...
            log.debug(f"ActionAutoClose time_since_last : {time_since_last}")
    ...
    def set_last(self):
        self.last_interaction = time.time()
        log.debug(f"ActionAutoClose set_last : {self.last_interaction}")
...
class ActionMenu(xbmcgui.WindowXMLDialog):
    ...
    def onAction(self, action):
        ...
        if action.getId() == 10:  # ACTION_PREVIOUS_MENU
            ...
            log.debug(f"ActionMenu: onAction: {action.getId()}")

    def onClick(self, control_id):
        if control_id == 3000:
            self.selected_action = self.listControl.getSelectedItem()
            log.debug(f"ActionMenu: Selected Item: {self.selected_action}")
            self.auto_close_thread.stop()
            self.close()
